...
def load_data()->(
        NamedTuple("Datasets", [("X_train", Dataset),("y_train", Dataset),("X_test", Dataset),("y_test", Dataset)])):
...
@dsl.component(base_image="python:3.11-slim",
    packages_to_install=["pandas", "scikit-learn", "xgboost", "mlflow"]
)
def train_model(X_train:Dataset, y_train:Dataset, X_test:Dataset, y_test:Dataset):
    ...
    for i,run in enumerate(results):
        with mlflow.start_run(run_name=excution_time+"_cv_"+str(i)):
            ...
            if i == max_run:
                ...
                print(f"Best model test metrics: f-1 score {f1}, precision {precision}, recall {recall }")
...
@dsl.pipeline(
    name="Credit-card-fraud-pipeline",
    description="ML pipeline that creates ml flow model."
)
def pipeline():
    data = load_data()
    train_model(X_train=data.outputs["X_train"], y_train=data.outputs["y_train"], X_test=data.outputs["X_test"],
